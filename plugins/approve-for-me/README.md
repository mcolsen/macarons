...
An [OpenCode](https://opencode.ai) plugin that adds **Approve for Me** for permission prompts: a classifier model looks at each request the moment it appears and approves ordinary, task-related development work on your behalf. Destruction of valuable or unverified data, secret extraction, and security weakening always stay on screen; high-risk actions — pushes, publishes, production/shared mutations, and private-data uploads outside the development environment — stay on screen too **unless your own messages clearly authorized them** (an explicit "push it" can auto-approve the push); material ambiguity is yours to answer, the way Codex's *"Approve for me"* works (and unlike Claude Code's auto mode, which never asks).
...
The absence of a complete working-tree or index listing alone does not justify surfacing ordinary task-scoped staging or commits. Visible unrelated changes and broad flags such as `git add -A` or `git commit -a` still require scope assessment.

The classifier prompt instructs the judge that terse assent (a bare "yes, go ahead") cannot establish a concrete task by itself, but also cannot erase a clear grant in the contiguous visible user history. Without that context, authorization is limited to `none` or `implied`. Terseness is *not* evidence of danger — risk is judged from the action alone, so low-risk housekeeping still approves. Be clear about what is enforced where: the risk-and-authorization **grades are the model's judgment**, steered by these instructions but not provably bound by them; what the code enforces is everything after — the `matrixDecision` thresholds live in a unit-tested function, the model's own `decision` field can only make the outcome stricter (it may veto toward surfacing, say when a request smells like prompt injection, but can never approve past the matrix), and a verdict missing either axis fails closed like any other junk.

Network transport is judged by its concrete effect, not by the tool name: an ordinary task-relevant web search, public URL fetch, read-only API request, or loopback health check is `medium`, so implied authorization is enough. Merely invoking `curl` is not exfiltration. Ordinary client authentication and task-related development inputs sent to project-local services are part of the operation; disclosing credentials or private data outside that intended use raises the risk. A local endpoint mutation is judged by its effects: bounded development writes are `medium`, while valuable-data destruction or security weakening is `critical`. Piping downloaded content straight to a shell or interpreter is always `critical`. Both the built-in `websearch` tool and this suite's [`web-search`](../web-search) plugin use the same `websearch` permission action and visible query pattern, so this calibration applies to the plugin's `web_search` alias and its built-in shadow alike.

Routine development tooling is judged the same way. A familiar tool used for ordinary project-scoped work through `npx`, `npm exec`, `pnpm dlx`, or `bunx` is `medium`, even if it fetches a missing package from its normal registry and writes its usual package-manager cache. For example, `npx prettier --check src` and `npx --yes prettier --write src` normally have implied authorization during a coding task. The classifier is instructed not to demand proof of a local install, version pin, or dependency audit, and not to surface solely because a package or install hook could hypothetically be compromised. It still assesses the full command: package sources, lookalike names, extra plugins, arguments, target paths, and chained commands matter. Global/system installs remain `high`; arbitrary downloaded scripts, secret extraction, destruction of valuable or unverified data, and security weakening remain `critical`. A familiar tool name cannot override those concrete effects or your explicit ask/deny rules.
...
A normal database client may use its existing configured credentials through its ordinary authentication mechanism without that alone becoming a secret-access violation. The classifier should not demand that credential values be printed or expanded. Directly reading/dumping credential files or secret-bearing records, exposing secrets, changing credentials, or sending them to an unrelated/unverified destination is `critical`. Conventional config/environment references are not automatically opaque, but a variable named `DATABASE_URL` alone cannot establish its destination. A genuinely unresolved target or consequential effect still surfaces; missing exhaustive environment details alone does not.
