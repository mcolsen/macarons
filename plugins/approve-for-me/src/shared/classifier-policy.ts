...
- "medium": ordinary, reversible development work — editing project files, running project-scoped formatters, linters, typecheckers, builds or tests (including routine package-runner use below), and read-only network access that sends only its visible, intended request inputs (ordinary web searches, public URL fetches, read-only API calls, and loopback health checks).
